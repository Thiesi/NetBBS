...
from netbbs.link.protocol import (
    ...
    RealtimeIdentityPayload, RealtimeProtocolVersionError,
    build_channel_message_frame, build_subscribe_frame,
)
...
def test_dial_authenticates_expected_peer_before_surfacing_version_mismatch():
    async def scenario():
        alice = bootstrap_node_identity("alice-version-identity-order")
        unrelated = bootstrap_node_identity("unrelated-version-identity-order")
        expected = bootstrap_node_identity("expected-version-identity-order")
        registry = LinkRealtimeSessionRegistry(own_fingerprint=alice.fingerprint)

        async def legacy_unrelated_responder(reader, writer):
            handshake = NoiseXXHandshake(
                initiator=False,
                static_private_key=bytes(derive_encryption_private_key(unrelated.transport_key)),
            )
            try:
                first = await read_realtime_record(reader)
                handshake.read_message(first)
                legacy_payload = replace(
                    RealtimeIdentityPayload.for_node(unrelated), realtime_protocol_version=1
                ).to_json_bytes()
                second, _ = handshake.write_message(legacy_payload)
                await write_realtime_record(writer, second)
            finally:
        ...
        server = await asyncio.start_server(legacy_unrelated_responder, "127.0.0.1", 0)
        port = server.sockets[0].getsockname()[1]
        try:
            ...
            assert not isinstance(exc_info.value, RealtimeProtocolVersionError)
