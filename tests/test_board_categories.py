"""Tests for netbbs.boards.categories — two-level category hierarchy."""
...
from netbbs.boards.categories import (
    CategoryError,
    create_category,
    delete_category,
    get_category_by_id,
...
def test_delete_category_sets_boards_using_it_to_uncategorized(db, creator):
    category = create_category(db, "Vintage Computing", created_by=creator)
    board = create_board(db, "general", category_id=category.id, creator=creator)
    delete_category(db, category, deleted_by=creator)
    updated = get_board_by_name(db, "general")
