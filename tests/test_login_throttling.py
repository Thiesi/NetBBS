...
def test_throttled_attempt_ends_login_with_throttled_message(monkeypatch):
    ...
    async def scenario() -> None:
        ...
        await login_flow.handle_session(session, object(), ChatHub(), PresenceRegistry(), throttle, config)
...
def test_unauthenticated_slot_is_released_after_login_completes(monkeypatch):
    ...
    async def scenario() -> None:
        ...
        await login_flow.handle_session(first_session, object(), ChatHub(), PresenceRegistry(), throttle, config)
