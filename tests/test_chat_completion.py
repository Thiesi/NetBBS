...
def test_bare_slash_lists_every_visible_command(db, lane, presence, alice, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
...
def test_command_prefix_matches_only_names_starting_with_it(db, lane, presence, alice, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
...
def test_command_completion_is_case_insensitive(db, lane, presence, alice, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    assert "/quit" in completer("/QU")


def test_non_moderator_does_not_see_moderation_commands_suggested(db, lane, presence, alice, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    candidates = completer("/m")
    assert "/mute" not in candidates


def test_moderator_does_see_moderation_commands_suggested(db, lane, presence, alice, channel):
    grant_permissions(
...
def test_moderation_commands_stay_hidden_from_completion_after_a_moderate_grant_elsewhere(
    db, lane, presence, alice, bob, channel
):
    ...
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, bob))
    assert "/mute" not in completer("/m")


def test_unrelated_command_prefix_matches_nothing(db, lane, presence, alice, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
...
def test_non_manager_does_not_see_membership_admin_commands_suggested(db, lane, presence, alice, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    candidates = completer("/")
...
def test_manager_does_see_membership_admin_commands_suggested(db, lane, presence, alice, channel):
    grant_permissions(
        db, alice, object_type="channel", object_id=channel.id,
...
def test_invite_is_suggested_to_a_plain_member_when_the_channel_opts_in(db, lane, presence, alice, bob, channel):
    ...
    completer = asyncio.run(chat_flow._build_completer(lane, presence, open_channel, bob))
    candidates = completer("/")
    assert "/invite" in candidates
...
def test_msg_completes_against_online_usernames(db, lane, presence, alice, bob, channel):
    presence.enter("bob")
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    assert completer("/msg bo") == ["bob"]


def test_msg_does_not_suggest_an_offline_user(db, lane, presence, alice, bob, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    assert completer("/msg bo") == []


def test_private_completes_the_same_way_as_msg(db, lane, presence, alice, bob, channel):
    presence.enter("bob")
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    assert completer("/private bo") == ["bob"]


def test_query_is_no_longer_a_recognized_command_prefix(db, lane, presence, alice, bob, channel):
...
def test_msg_completion_is_case_insensitive(db, lane, presence, alice, bob, channel):
    presence.enter("bob")
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    assert completer("/msg BO") == ["bob"]


def test_msg_completion_stops_once_a_full_argument_is_typed(db, lane, presence, alice, bob, channel):
    presence.enter("bob")
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
...
def test_whois_completes_against_offline_users_too(db, lane, presence, alice, bob, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    assert completer("/whois bo") == ["bob"]


def test_finger_completes_against_offline_users_too(db, lane, presence, alice, bob, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    assert completer("/finger bo") == ["bob"]


def test_whois_completion_matches_multiple_candidates(db, lane, presence, alice, bob, carol, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
...
def test_invite_completes_against_offline_users_too(db, lane, presence, alice, bob, channel):
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
    assert completer("/invite bo") == ["bob"]


def test_invite_excludes_users_already_a_member(db, lane, presence, alice, bob, channel):
    from netbbs.chat.membership import add_member
    from netbbs.moderation import ChannelPermission, grant_permissions
...
def test_plain_chat_text_has_no_candidates(db, lane, presence, alice, bob, channel):
    presence.enter("bob")
    completer = asyncio.run(chat_flow._build_completer(lane, presence, channel, alice))
