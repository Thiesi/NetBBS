...
def test_me_shows_action_to_the_actor(db, hub, presence, alice, channel):
    async def scenario():
        session = FakeSession(["/me waves", "/quit"])
        await asyncio.wait_for(chat_flow._chat_loop(session, db, hub, presence, channel, alice), timeout=2)
        return session
...
def test_me_is_broadcast_to_others(db, hub, presence, alice, bob, channel):
    async def scenario():
        bystander = FakeSession()
        bystander_task = asyncio.create_task(chat_flow._chat_loop(bystander, db, hub, presence, channel, bob))
...
def test_me_is_recorded_as_an_action_in_scrollback(db, hub, presence, alice, channel):
    async def scenario():
        session = FakeSession(["/me waves", "/quit"])
        await asyncio.wait_for(chat_flow._chat_loop(session, db, hub, presence, channel, alice), timeout=2)
...
def test_me_with_no_action_text_shows_usage(db, hub, presence, alice, channel):
    async def scenario():
...
def test_me_replays_correctly_from_scrollback(db, hub, presence, alice, channel):
