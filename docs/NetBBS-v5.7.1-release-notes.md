...
network's colors, answers CTCP, and lets callers ask the hub things from
inside a bridged channel. There are no database migrations and no protocol
version changes.

## The wire convention, fixed

Every MRC client embeds the sender's own colored handle inside the message
...
line now goes out as `<handle> text` in the network's color codes, and an
inbound line is recorded under one name, as a message or as an action.

## Colors

MRC users color their lines with Mystic-style `|NN` codes. NetBBS now shows
those colors in bridged channels, live and in scrollback, after the usual
sanitization. Callers who prefer plain text switch them off under
`[P]rofile` → `[I]nter-BBS chat colors`. Typed codes in a caller's own lines
