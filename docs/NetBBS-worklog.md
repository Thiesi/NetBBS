...
`LinkNode.known_identity` and `resolve_*` verification lookups consult both.
A new check of "do I know this node" has to decide which question it is
asking. The display layer reads the `link_known_identities` view so that names
and the familiar-name warning cover introduced nodes; nothing that decides
authority may read that view.
...
used for a carrier's inventory response, treats the two differently. The
refusal sites were converted by message, so a new refusal of the first kind
has to raise the subclass or it rejects whole responses again. The inventory
...
`services/reliable_nodes/reliable-nodes.json` is dialed as both seed and relay
by every node whose SysOp accepted Link participation, and it is a hand-edited
static file. When the one node it listed had its own Link participation
switched off, nothing detected it: the roster is not generated by anything that
could check it, and on the node side a failed seed dial logs exactly like
ordinary churn. Two separate signals now exist, and neither replaces the other.

Project side, `services.reliable_nodes.check_roster` probes each entry by
POSTing an unparseable hello to `{url}/link/v1/hello` and expecting the HTTP 400
`malformed hello` that `LinkServer._handle_hello` returns. That is the useful
liveness probe precisely because it stops short of a real hello: it needs no
...
Node side, `run_link_sync` counts *consecutive* passes that reached nothing at
all — no seed, no roster node, no fallback candidate — and warns every third.
Per-pass failures stay at their existing level; only the consecutive count is
promoted, because "one seed is flaky" and "this node is cut off entirely" are
different states and only the second is worth a SysOp's attention. Being a
...
therefore runs in the configuration its own warning discourages. Both texts are
correct as written; whether the warning should acknowledge the roster case is
an open design-doc decision, not something to patch into the warning text.
...
each page's slice of a group is keyed for itself, so a letter always means what
the page printing it says even when an entry spans pages. Page movement goes
through `page_step`, which clamps at both ends and returns `None` for anything
that is not a paging key; it takes the key pair a screen advertises, because `N`
is a live action on screens that do not page with it.

Two screens assign their keys per page rather than per entry -- the mission board
and `_pick_trade_field` both number the choices visible on the page, up to nine --
which `paginate`'s fixed group keys do not model, so they fill their own pages.
They still measure with `page_capacity`'s rule and clamp with `page_step`. A new
paged screen that measures or clamps for itself will disagree with the rest at
some width.
