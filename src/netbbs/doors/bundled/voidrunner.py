...
def new_tactics(pirate: Pirate, version: int = TACTICAL_RULESET_VERSION) -> dict:
    ...
    # `brace_ready` is the saved field; the player-facing verb is Guard (#400).
...
def squadron_terms(world: World) -> list[str]:
    ...
    lines = [f"Squadron: +{squadron_cover(world)} cover; target {target['name']} (T{target['tier']}).",
             "Cover adds to incoming damage before Guard.",
             f"Partner: {wing['name']}, tier {wing['tier']}, HP {wing['hp']}/{wing['hp_max']}; {new_tactics(Pirate(**wing))['profile']} pattern.",
             "Destroy the target to prevent this return volley and end covering fire. Guard reduces combined damage.",
...
def tactical_round(world: World, pirate: Pirate, tactics: dict, action: str) -> tuple[int, int, list[str]]:
    if action not in ("F", "G") or (action == "G" and not tactics["brace_ready"]):
        raise ValueError("Fire to recharge Guard before using it again.")
    ...
    lines = [f"You hit the {pirate.name} for {damage} damage." + (" Guarding; fire to recharge." if action == "G" else "")]
    if pirate.hp > 0:
...
# [S] Scan, [G] Go to, [V] View are fixed control keys on this same prompt,
...
# collide with (and be permanently shadowed by) the "[G] Go to" hotkey,
...
def table(headers: list[str], rows: list[list[str]], aligns: str = "",
    ...
    name per cell, defaulting to a slate header and ink body. `optional` lists
    column indices in the order they may be dropped when the table will not fit
    the page -- a narrow terminal loses the least useful column rather than
    letting every row wrap into an unreadable stack.
    """
    width = _page_content_width() if width is None else width
    columns = len(headers)
    aligns = (aligns + "l" * columns)[:columns]
    keep = [index for index in range(columns)
    ...
    while True:
        ...
        if sum(widths) + gap * (len(keep) - 1) <= width or not optional:
            break
    ...
    separator = " " * gap
    heading = separator.join(
        _pad(_cell_style("label", headers[index]), widths[position],
             "right" if aligns[index] == "r" else aligns[index] == "c" and "center" or "left")
    ...
    for number, row in enumerate(rows):
        ...
        for position, index in enumerate(keep):
            ...
            cells.append(_pad(row[index] if ANSI_ESCAPE_RE.search(row[index]) else _cell_style(role, row[index]),
                              widths[position], align))
        body = separator.join(cells).rstrip()
        out_rows.append(table_member(body) if repeat_header else body)
    return out_rows
...
def market_catalog_lines(world: World, goods: list[str]) -> list[str]:
    ...
    lines += table(["COMMODITY", "BUY", "SELL", "STOCK", "DEMAND", "HELD", "SPREAD", ""],
                   rows, "lrrrrrll", styles=styles, optional=(3, 4, 6, 7))
...
def paginate(groups: list[list[str]], capacity: int, *, render=None, keys=None):
    ...
    """
    held = [row for rows in groups for row in rows if STICKY_MARK in row]
    pages: list[tuple[list[str], dict]] = [([], {})]

    def room_on(page_index: int) -> int:
        # The first page carries its headings in the flow, where the screen put
        # them; every later page is given them back, and pays for them.
        return capacity if page_index == 0 else max(1, capacity - len(held))
    ...
    if held:
        for page_index in range(1, len(pages)):
            ...
            if not any(MEMBER_MARK in row for row in rows):
                continue  # not a page of the table; its headings would be a lie
            missing = [row for row in held if row not in rows[:len(held)]]
            pages[page_index] = (missing + rows, choices)
...
def wrapped_group(line: str) -> list[str]:
    ...
    return wrap_styled(style_body_line(line), _page_content_width())
...
def trade_route_lines(world: World, destination: int | None, commodity: str, quantity: int, use_hold: bool) -> list[str]:
    ...
    lines = [f"Destination: {name}. Cargo: {COMMODITIES[commodity]['label']} x{quantity}.",
             ...
             "Use [E] Edit draft to change the route. Back makes no changes; this estimate never buys cargo or launches a route."]
...
def _pick_trade_field(title: str, options: list[tuple[object, str]], *, max_choices: int = 9, notice: list[str] | None = None, page_state: dict[str, int] | None = None) -> object | None:
    footer = f"[1-{max_choices}] Select [N] Next [P] Prev [B] Back: "
...
def edit_door_draft(p: "Palette", *, title: str, initial: dict, fields: list[tuple],
    ...
    draft = dict(initial)
    page, error = 0, None
    footer = " ".join(f"[{key}] {label}" for key, label, _, _ in fields) + " [S] Apply [N] Next [P] Prev [B] Back: "
    while True:
...
def screen_trade_route(p: Palette, world: World, *, initial: dict | None = None) -> None:
    ...
    page = 0
    footer = "[E] Edit draft [N] Next [P] Prev [B] Back: "
    while True:
...
def screen_economy_opportunities(p: Palette, world: World) -> None:
    ...
    footer = "[1-6] Route [N] Next [P] Prev [B] Back: "
...
# Every hotkey in the game is written `[K] Label` (issue #400), so the paging
# tokens have one spelling each; they are still matched by shape rather than by
# whole-footer text, because a footer ends with a space, the prompt's own colon
# or " > " (issue #412 review).
_PAGING_PATTERN = re.compile(r"\[<\] Prev \[>\] Next"
                             r"|\[N\] Next \[P\] Prev(?:ious)?"
                             r"|\[P\] Prev(?:ious)? \[N\] Next"
                             r"|\[<>\] Page"
                             r"|\[<\] \[>\]")
...
def screen_shipyard(p: Palette, world: World) -> None:
    page, result = 0, None
    footer = "[<] Prev [>] Next [R] Refuel [P] Repair [K] Crew [S] Specialists [V] Ship [B] Back: "
    while True:
...
def crew_roster_lines(world: World) -> list[str]:
    ...
    drawn = table(["", "", "", "", "", ""], heads, "llllll", styles=styles,
                  optional=(4, 2), repeat_header=False)[1:]
    lines: list[str] = []
    for row, (index, role, hired, level, name, personality) in zip(drawn, meta):
        lines.append(row)
...
def screen_crew(p: Palette, world: World) -> None:
    page, result = 0, None
    footer = f"[<>] Page [1-3] Task [{letter_span(CREW_LETTERS[:len(CREW_ROLES)])}] Crew [B] Back: "
    while True:
...
def pilot_guide_lines(world: World) -> list[str]:
    ...
    if active is not None:
        ...
        lines.append("Optional First Flight: Freeport merchants will sponsor one local delivery. [O] Offer shows all terms before acceptance.")
    else:
    ...
    lines += [
        "1. Buy cargo: [M] Market, choose the commodity's letter, [P] Purchase, enter a quantity. Buying spends credits and needs free hold space. Enter with no quantity cancels.",
        "2. Keep fuel: [Y] Yard, [R] Refuel. Each fuel unit costs 6 cr. Reserve enough for the outward and return jumps; keep credits for repairs and crew wages too.",
        "3. Depart: [C] Chart, select the destination's letter, then confirm its fuel cost and danger. A jump advances one day, uses fuel and charges wages. Browsing, trading and upgrades do not advance the day.",
        "4. Deliver a contract by docking with its full cargo. Ordinary trading instead uses [M] Market, commodity letter, [S] Sell. Sales pay less than the local buy quote; distant prices can change.",
...
def _opening_next_step(world: World, offer: Mission) -> str:
    ...
    if not missing:
        return f"[C] Chart to {station}; all {plural(offer.quantity, 'unit')} of {label} are already aboard."
    return f"[M] Market, buy {plural(missing, 'unit')} of {label}, then [C] Chart to {station}."
...
def _screen_opening_offer(p: Palette, world: World, offer: Mission) -> bool:
    ...
    lines += [f"Reserve {2 * fuel} fuel for delivery and return; {world.save.ship.fuel} aboard. Fuel replacement costs {12 * fuel} cr for both jumps.",
              ...
              "After acceptance, use [M] Market to buy the goods, [Y] Yard to refuel if needed, then [C] Chart to jump to the named station."]
...
def screen_mission_navigation(p: Palette, world: World, mission: Mission, *, active: bool) -> None:
    ...
    while True:
        if pages is None:
            live = active and any(m.id == mission.id for m in world.save.active_missions) and not mission_expired(world, mission)
            footer = ("[J] Jump next " if live and not (mission.kind == "scan" and world.by_id[mission.target_system].discovered) and mission_route(world, mission) else "") + "[V] Map [N] Next [P] Prev [B] Back: "
...
def hall_of_fame_lines(entries: list[dict], user_id: int) -> list[str]:
    ...
    for position, entry in enumerate(entries, 1):
        ...
        rows.append([
            ...
            str(entry.get("rank", "?")),
            f"{p.gold}{entry.get('best_credits', 0):,}{RESET}",
    ...
    lines = table(["", "PILOT", "RANK", "BEST CR", "", "WINS", "JOBS", "RUNS", ""],
                  rows, "rllrlrrrl", styles=styles, optional=(2, 7, 6, 4))
    lines.append(f"{p.slate}Best recorded credits. Wins are the latest career's combat victories; "
                 f"jobs are missions completed; runs are retirements.{RESET}")
    return lines
...
def danger_dots(level: int, *, known: bool = True) -> str:
    ...
    p = pal()
    if not known:
        return f"{p.slate}{glyph('crew_off') * 5}{RESET}"
...
def chart_entries(world: World, result: str | None = None) -> list[tuple[int | None, str]]:
    ...
    if result:
        entries.append((None, alert("note", "Result", _mission_plain(result))))
    ...
    for sid in sorted(here.connections):
        ...
        ids.append(sid)
    drawn = table(["DESTINATION", "AT", "SECTOR", "ECONOMY", "DANGER", "FUEL", ""], rows,
                  "lllllrl", styles=styles, optional=(1, 2, 3),
                  width=max(1, _page_content_width() - 4))
    # The heading sits under the same key prefix its rows will carry, and keeps
    # its sticky mark so it is repeated on every page of the departures.
    entries.append((None, STICKY_MARK + " " * 4 + drawn[0].replace(STICKY_MARK, "")))
    entries += list(zip(ids, drawn[1:]))
    return entries
...
def _chart_pages_for(world: World, title: str, footer: str, result: str | None):
    ...
    wrapped = [(sid, [SECTION_MARK + text[1:]] if text.startswith(SECTION_MARK)
                else wrap_styled(style_body_line(text),
                                 max(1, _page_content_width() - (4 if sid is not None else 0))))
...
def screen_auto_route(p: Palette, world: World, *, destination: int | None = None) -> None:
    ...
    while True:
        if pages is None:
            ...
            footer = ("[J] Jump next " if path else "") + "[D] Destination [V] Map [N] Next [P] Prev [B] Back: "
...
# Every combat key is its verb's own initial, and the bar's label is the word the
# detail line leads with, so the two screens never name one action twice (#400).
# `B` cannot be Brace, because `B` is Back everywhere and a stray Back must not
