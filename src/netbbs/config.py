...
def is_node_display_name_placeholder(db: Database) -> bool:
    """Whether this node still shows the shipped default name (design doc
    §16, issue #219 Decision 6): a node may run locally with it forever,
    but may not participate in NetBBS Link until a SysOp picks a real one
    -- every default-named node on a mesh would be indistinguishable in
    conversation. Compared case-insensitively so "netbbs" typed back in
    by hand doesn't count as a choice either."""
    return get_node_display_name(db).strip().lower() == _DEFAULT_NODE_DISPLAY_NAME.lower()
